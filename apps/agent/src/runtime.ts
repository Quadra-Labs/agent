// runtime.ts — standalone ElizaOS runtime boot for the agent app (A3, Task 1).
//
// Lifts the proven demo boot (demo/src/runtime.ts), which itself mirrors the
// phase0 spike: AgentRuntime + manual migration bootstrap + config via
// character.settings (NOT process.env) + a deterministic local embedding handler.
// The ONLY change vs. the demo is the plugin set: this app registers ALL FOUR
// plugins so both the Walrus and MemWal services come up live.
//
// Plugin set (order matters): [sqlPlugin, groqPlugin, walrusPluginWithSigner,
// memwalPlugin]. SQL first so the DB adapter type is known before initialize().
// walrusPluginWithSigner is registered BEFORE memwalPlugin so the long-lived
// WalrusService exists when MemwalService resolves it (dependency direction
// MemWal -> Walrus; Walrus/Seal never depend on MemWal).
//
// EMBEDDING NOTE (lifted from the demo): plugin-groq registers no TEXT_EMBEDDING
// handler. plugin-sql's createMemory only writes/searches an embedding when one is
// present, so plain chat/index writes do not strictly need it — but to keep memory
// writes working WITHOUT a second API key we register a deterministic, fully local
// embedding handler: a fixed-dimension vector hashed from the input text. It is
// NOT semantically meaningful; it exists only so the runtime never depends on an
// external embedding service. No network, no key.
//
// This module is BOOT + SERVICE-RESOLUTION ONLY (Task 1). It does NOT build chat
// memory, the checkpoint writer, recall, or summarization (Tasks 3-6).
