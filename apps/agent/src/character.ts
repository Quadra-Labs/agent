// character.ts — custom-agent definition + loader for the interactive CLI.
//
// A "character" is the small, user-authorable identity the runtime boots under:
// its name (which is ALSO the checkpoint index `agent` key), its bio, an OPTIONAL
// system-prompt override for the chat loop, and an OPTIONAL list of job-template
// category ids it offers. Everything here is OPTIONAL on top of a DEFAULT_CHARACTER
// that reproduces the original hardcoded "WalrusAgent" identity, so code paths that
// do not pass a character (the eight proofs) behave byte-identically.
//
// Characters live under apps/agent/characters/ as EITHER a Markdown file (YAML
// frontmatter + body, the friendlier hand-authoring format — see
// characterMarkdown.ts) OR a plain JSON file. loadCharacter reads + validates one and
// returns a typed ok/kind result — NEVER a blind throw — mirroring the failure style
// of templates.ts / the plugins. Both formats converge on the SAME parseCharacter
// validator, so they share one shape and one set of error kinds. The framework does
// not author behavior here; a character only NAMES an identity and (optionally)
// selects which templates the agent reasons over. The job-intake RULES live in chat.ts.
