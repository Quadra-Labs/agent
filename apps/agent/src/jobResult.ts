// jobResult.ts — write the FINISHED job result to Seal (encrypted) on Walrus.
//
// This is the deliberately SMALL app-level Seal write path (NOT a plugin). It runs
// ONLY when a job is done: the agent produces the real result, encrypts it under the
// on-chain quadra::job_access policy, and stores the ciphertext on Walrus. Nothing
// here happens mid-conversation; completeIntake (intakeNotification.ts) calls it once,
// after every parameter is collected and the notification is constructed.
//
// THE ON-CHAIN BINDING (quadra::job_access, contracts/sources/job_access.move):
//   intake::pay_for_job records `{ job_id -> (user, agent) }`. seal_approve(id, reg)
//   does `string::utf8(id)` and lets the user / agent / scheduler decrypt. So the Seal
//   IDENTITY must be the UTF-8 job_id. @mysten/seal hex-decodes the `id` string
//   (fromHex), so we pass id = toHex(utf8Bytes(job_id)). Encrypting under any other id
//   would store durable bytes that NO ONE can decrypt through the deployed policy.
//   Access binds to the encrypt-time id, not the PTB (memory: seal-id-binds-access).
//
// THREE STEPS, each a typed ok/kind union — NEVER a blind throw for an expected
// failure, and NEVER logging the signer/result bytes:
//   1. produceResult     — one structured model call -> an object matching the
//                          template's job_template.output schema (validated; off-schema
//                          is invalid_result).
//   2. sealEncryptResult — Seal-encrypt the result JSON under id = hex(job_id),
//                          packageId = the deployed quadra package, open-mode servers.
//   3. storeJobResult     — produce -> encrypt -> walrus.store(ciphertext). The
//                          single entry point completeIntake calls.
...
// apps/agent depends on the runtime CONTRACT (getService("walrus")), not the plugin class.
type WalrusLike = {
