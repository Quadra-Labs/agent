// intakeNotification.ts — CONSTRUCT (never sign/send) the Intake notification when a
// job has all parameters collected. buildIntakeNotification is a pure builder;
// completeIntake extracts the collected values from the transcript via one structured
// useModel call (leak-guarded), builds the notification, and logs it. The job_template
// field is real ({ output, lifetime }); the other three fields are PROVISIONAL stubs
// until the cross-workstream identity/job-id interfaces are locked.
...
// plugin-groq returns this sentinel instead of throwing; treat it as a hard extraction
// failure so we never build a notification from a fake extraction.
