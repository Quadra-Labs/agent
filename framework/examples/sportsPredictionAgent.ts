// sportsPredictionAgent.ts — a Polymarket PRICE specialist for the SPORTS domain (NBA, NFL,
// soccer, and the like; evaluator polymarket-price, competition category "prediction"). Given a
...
//
// Like cryptoCommoditiesPredictionAgent.ts it anchors on the live market price and tilts by the
// recent CLOB momentum (damped over the forecast horizon), but it DROPS the crypto-spot cross-check:
// a sports market (a game outcome) has no underlying spot price to compare against. The signals
// (see cryptoSignals.ts) reduce to two: the live YES anchor plus the damped, capped CLOB drift.
...
/**
 * forecast_sports_probability — forecast a sports Polymarket market's YES probability at
 * `targetTs` (unix seconds). Combines two signals into one probability in [0,1]:
 ...
 * There is no spot cross-check: a sports market has no underlying spot price.
 * Returns only { probability }, matching the polymarket-price template's output schema.
 */
export const forecastSportsProbability = defineSkill({
  name: "forecast_sports_probability",
  description:
    "Forecast a sports Polymarket market's YES probability at a target date from the live market price and the recent CLOB momentum.",
  input: z.object({
...
export const sportsPredictionAgent = defineAgent({
  name: "SportsForecaster",
  bio: [
    "I forecast the YES probability of sports prediction markets and deliver it sealed.",
    "Give me a Polymarket market id (an NBA, NFL, soccer, or similar sports market) and how long to run (e.g. '10 minutes'), and I return a probability in [0,1].",
  ],
  systemPrompt: [
    "You are SportsForecaster, a specialist that sells ONE job: a YES-probability (price)",
    "forecast for a SPORTS Polymarket market over a window you choose. This is the 'polymarket-price'",
    "job; you do not sell market resolution or whole-event guesses.",
    "Rules you MUST follow:",
    "- You only take sports markets (NBA, NFL, soccer, baseball, tennis, and the like).",
    ...
    "  'Accepted: polymarket-price forecast for market <id>, asset SPORT, lifetime <Nm>, price 10 QUADRA.'",
  ...
  skills: [forecastSportsProbability],
});

export default sportsPredictionAgent;
